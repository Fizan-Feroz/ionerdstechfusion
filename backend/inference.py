...
class RiskScoreEngine:
    ...
    def add_vital(self, patient_id, vital_dict):
        ...
        with self.lock:
            ...
            features = ['HR', 'RespRate', 'Temp', 'NISysABP', 'NIDiasABP']
            vital_vec = []
            for f in features:
                if f in vital_dict:
            ...
            if len([v for v in vital_vec if not np.isnan(v)]) > 0:
    ...
    def _compute_risk_score(self, patient_id):
        ...
        if self.model is None:
            ...
            recent_vitals = list(buffer)[-1]  # last reading
            score = 30  # baseline
        ...
        try:
            ...
            # Normalize
            X = (X - np.nanmean(X, axis=0)) / (np.nanstd(X, axis=0) + 1e-6)
            ...
            with torch.no_grad():
                x_tensor = torch.tensor(X.reshape(1, -1, X.shape[1]), dtype=torch.float32)
